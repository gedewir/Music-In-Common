...
//converts Spotify profile URL to Spotify ID, used further on in app.get('/')
function linkToID(link){
    //seperates link to string arrays if it contains backslash '/'
    let splitted = link.split("/");
    //if username contains '?', split it and only use the username
...
const getMatchedTracks = async(playlistArray, users,access_token,callback)=>{
    ...
    var uniqueTrackIDs = [];
    var uniqueTracks = [];
    //checks to see if matched tracks are duplicates by filtering the track ID
    const unique = matchedTracks.filter(element => {
        //returns boolean (true means not duplicate, false means it is duplicate) whether track is duplicate by checking if selected element is contained in uniqueTracksID array 
        const isDuplicate = uniqueTrackIDs.includes(element.id);
    
        if (!isDuplicate) {
            //append track ID only
            uniqueTrackIDs.push(element.id);
            //append all track information
